...
public class EnemySpawn : MonoCache
{
    ...
    [SerializeField] private float[] _timerSpawnWave;
    ...
    public float TimeWave
    {
        ...
        set
        {
            ...
            else if(_timeWave <= 0)
            {
                ...
                {
                    if (_enemiesOnScene[i].GetComponent<EnemyHealth>() != null)
                    {
                        _enemiesOnScene[i].GetComponent<EnemyHealth>().UpgradeHealthMax(1.05f);
                    }
                    else
                    {
                        _enemiesOnScene[i].GetComponent<EnemyMove>().EnemyHealthUpgrade(1.05f);
                    }
                }
                _calm = false;
        ...
        for (int i = 0; i < _enemiesOnScene.Count; i++)
        {
            if(_enemiesOnScene[i].activeSelf == false && i >= type && i < type + _count)
            {
                ...
                if (enemyObj.GetComponent<EnemyMove>() != null)
                {
                    ...
                    enemyObj.GetComponent<EnemyMove>().EnemyHealthSound();
                }
                else if(enemyObj.GetComponent<EnemyFlyMove>() != null)
                {
                    ...
                    enemyObj.GetComponent<EnemyHealth>().Begin();
                }
                break;
            }
        }
    ...
    public void SpawnEggsEnemy(int typeEnemy, int Line, Vector3 pos)
    {
        int type = typeEnemy * _count;
        for (int i = 0; i < _enemiesOnScene.Count; i++)
        {
            if (_enemiesOnScene[i].activeSelf == false && i >= type && i < type + _count)
            {
                ...
                if (enemyObj.GetComponent<EnemyMove>() != null)
                {
                    ...
                    enemyObj.GetComponent<EnemyMove>().NewWayEgg(_enemySystem.EnemyWayPoint(Line));
                    enemyObj.GetComponent<EnemyMove>().EnemyHealthSound();
                }
                else if (enemyObj.GetComponent<EnemyFlyMove>() != null)
                {
                    enemyObj.transform.position = pos;
                    enemyObj.GetComponent<EnemyFlyMove>().NewWayEgg(_enemySystem.EnemyWayPoint(Line));
                    enemyObj.GetComponent<EnemyHealth>().Begin();
                }
                break;
            }
